...
| Step 1         | ...knowing the identity of the authenticated Ethics Committee Member? (AC4)                                                     | AuthenticationRepository           | **Pure Fabrication**: the authentication repository (wrapping the AuthLib) owns user identity and role information; the controller queries it to associate the ValidationRecord with the correct member.                     |
...
| Step 4         | ...creating the ValidationRecord with the member identity and validationDate? (AC4)                                             | ValidateDeclarationController   | **Creator** + **Controller**: the controller orchestrates the creation of the ValidationRecord, supplying the member (from AuthenticationRepository) and the current date.                  |

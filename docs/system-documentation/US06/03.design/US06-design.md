...
| Step 1         | ...knowing the identity of the authenticated Political Agent? (AC5)                                                                 | AuthenticationService           | **Pure Fabrication**: the auth service owns user identity and role information; the controller queries it to associate the declaration with the correct agent.                      |
...
| Step 3         | ...providing the list of registered functions so the agent can associate them with position entries?                                | FunctionRepository              | **Pure Fabrication** + **Information Expert**: the repository holds all Function instances and knows how to retrieve them.                                                         |
| Step 3         | ...creating each PositionEntry with its nature, grossSalary, sideIncome, startDate, endDate, organization and function? (AC3, AC4) | Declaration                     | **Creator** + **Information Expert**: a Declaration aggregates its PositionEntry instances; it is responsible for creating and owning them.                                        |

...
* `PositionEntry` holds `grossSalary: Double` (annual gross salary), `sideIncomeConsulting: Double` (consulting side income, can be zero), and `sideIncomeBoardMemberships: Double` (board membership income, can be zero), in line with the declaration dataset structure (Table 2). It also holds `startDate: Date` and `endDate: Date`, and references a pre-existing `Function` and `Organization`. The nature of the position is represented by the `PositionNature` enum with values `PUBLIC`, `PRIVATE`, and `SOCIAL`.
* `AssetEntry` now covers three distinct asset categories, each with its own value: real estate (`RealEstate` with `description: String` and `municipality: String`), vehicles (`VehicleAsset` with `description: String`), and stocks (`StockAsset` with `description: String`). Each category value can be zero. The `assetValue: Double` is held on `AssetEntry` and the `AssetType` enum (`REAL_ESTATE`, `VEHICLES`, `STOCKS`) classifies which category it belongs to.
...
* `Organization` and `Function` are pre-existing entities registered in the system (dependencies on US04 and US05). The type of organization is represented by the `OrganizationType` enum with values `COMPANY`, `POLITICAL_PARTY`, `FOUNDATION`, `INSTITUTE`, and `ASSOCIATION`, consistent with US03 and US04.

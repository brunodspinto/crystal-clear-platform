...
|----------|-------------|
| `load_data(path)` | Reads the CSV with `pd.read_csv` and returns a `DataFrame` with columns `name`, `role`, `total_income` |
...
| `plot_boxplot(df, output_path)` | Saves a boxplot SVG using `df.groupby("role").boxplot()`; shows median, Q1-Q3, whiskers up to 1.5*IQR and outliers |
...
- `src/test/python/us16/test_income_boxplot.py` - 9 unit tests covering `load_data`, `summarise` and `quartis`. `plot_boxplot` is excluded from unit testing because it produces a binary SVG artefact - it is exercised manually via the script's `__main__` block.
...
The boxplot SVG is written to `docs/system-documentation/US16/us16_boxplot.svg`.
...
- [x] 9 unit tests for `load_data`, `summarise` and `quartis`

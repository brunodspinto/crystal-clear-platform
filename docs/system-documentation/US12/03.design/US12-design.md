...
| Step 3         | ...creating the (empty) complaint for the selected agent and the authenticated citizen? | ComplaintRepository | **Creator**: the repository records all Complaint instances, so it instantiates them (`createComplaint(citizen, agent)`); the controller only coordinates &mdash; it resolves the citizen + agent and delegates.                |
...
| Step 5         | ...requesting and keeping the grievance data (function, description, date) until confirmation? | SubmitComplaintUI | **Pure Fabrication**: the UI handles user interaction and temporarily keeps the inputs until submission.                                                                       |
| Step 6         | ...showing the grievance summary and requesting confirmation? | SubmitComplaintUI  | **Pure Fabrication**: the UI handles user interaction.                                                                                                       |
...
| Step 8         | ...asking whether to add another grievance about the same agent (loop)? | SubmitComplaintUI | **Pure Fabrication**: the UI drives the dialogue with the actor.                                                                                                         |
...
| Step 10        | ...informing operation success?                        | SubmitComplaintUI         | **Pure Fabrication**: the UI handles user interaction.                                                                                                       |

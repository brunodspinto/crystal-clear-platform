...
* The `PoliticalFunction` referenced in the complaint represents the role held by the political agent at the time of the behaviour, selected from the predefined enum `{minister, deputy, councillor, parishCouncilPresident, mayor}`, consistent with the global domain model.

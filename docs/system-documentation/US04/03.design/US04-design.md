...
| Step 7         | ...instantiating a new Organization?                                                              | OrganizationRepository          | **Creator**: the repository records (aggregates) all Organization instances, so by GRASP it instantiates them (`registerOrganization`); the controller only delegates and does not create the Organization itself. |
